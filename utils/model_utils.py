...
class FocalLoss(nn.Module):
    ...
    def __init__(self, alpha=1.0, gamma=2.0, reduction='mean', weight=None, label_smoothing=0.0):
...
class WeightedCrossEntropy(nn.Module):
    ...
    def __init__(self, weight=None, label_smoothing=0.1, reduction='mean'):
...
class ComboLoss(nn.Module):
    ...
    def __init__(self, alpha=0.5, beta=0.5, weight=None, label_smoothing=0.1):
        super(ComboLoss, self).__init__()
        self.alpha = alpha
        self.beta = beta
        self.ce_loss = WeightedCrossEntropy(weight=weight, label_smoothing=label_smoothing)
...
def get_criterion(criterion):
    ...
    if loss_fn == 'ce':
        ...
        weights = torch.tensor(criterion.weight, dtype=torch.float).cuda()
        return FocalLoss(weight=weights, label_smoothing=criterion.label_smooth)
    elif loss_fn == 'combo_loss':
        return ComboLoss()
...
def get_model_from_yaml(yaml_args):
    ...
    if model_name == 'AB_MIL':
        ...
        mil_model = DGR_MIL(yaml_args.Model.in_dim, yaml_args.General.num_classes, yaml_args.Model.L, yaml_args.Model.D, yaml_args.Model.n_lesion, yaml_args.Model.attn_mode, yaml_args.Model.dropout_node, yaml_args.Model.dropout_patch,
                            yaml_args.Model.initialize)
        return mil_model
