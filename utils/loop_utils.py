...
def train_loop_clinical(device, model, loader, criterion, optimizer, scheduler=None):
    start = time.time()
    model.train()
    train_loss_log = 0
    ...
    for i, data in enumerate(loader):
        ...
        bag = data[0].to(device).float()
        clinical = data[1].to(device).float()
        label = data[2].long().to(device)

        output = model(bag, clinical)
...
def val_loop_clinical(device, num_classes, model, loader, criterion, return_WSI_feature=False, return_WSI_attn=False):
    model.eval()
    val_loss_log = 0
    labels = []
    bag_predictions_after_normal = []
    ...
    with torch.no_grad():
        for i, data in enumerate(loader):
            ...
            if return_WSI_feature:
                output = model(bag, clinical, return_WSI_feature=True)
                WSI_feature = output['WSI_feature']
                WSI_features.append(WSI_feature.cpu().numpy())
                continue

            if return_WSI_attn:
                output = model(bag, clinical, return_attention=True)
                WSI_attn = output['WSI_attn']
                WSI_attns.append(WSI_attn.cpu().numpy())
                continue
            ...
            bag_predictions_after_normal.append(torch.softmax(val_logits, 0).cpu().numpy())
            val_logits = val_logits.unsqueeze(0)  # 恢复形状

            val_loss = criterion(val_logits, label)
            val_loss_log += val_loss.item()

    if return_WSI_feature:
        WSI_features = np.concatenate(WSI_features, axis=0)
        return WSI_features  # 与原始代码一致

    if return_WSI_attn:
        return WSI_attns  # 与原始代码一致
...
def clam_val_loop(device, num_classes, model, loader, criterion, bag_weight, retrun_WSI_feature=False, return_WSI_attn=False):
    ...
    labels = []
    bag_predictions_after_normal = []
    all_logits = []  # 新增：保存所有logits用于阈值优化
    all_labels = []  # 新增：保存所有标签
    model = model.to(device)
    WSI_features = []
    WSI_attns = []
    ...
    with torch.no_grad():
        for i, data in enumerate(loader):
            label = data[1].to(device).long()
            labels.append(label.cpu().numpy())
            all_labels.append(label.cpu().numpy())  # 新增
            bag = data[0].to(device).float()
