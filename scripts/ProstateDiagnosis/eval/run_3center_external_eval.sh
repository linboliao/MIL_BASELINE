#!/bin/bash
# External test evaluation (301 + YNZL) using the 3-center (v3) checkpoints.
set -e
...
for fold in 1 2 3 4 5; do
  ...
  for ext in 301 ynzl; do
    ...
    CUDA_VISIBLE_DEVICES=${gpu} ${PY} -u test_mil.py \
      ...
      --device cuda:0 \
      > "logs/ProstateDiagnosis/DataAnalysis/AB_MIL_uni2_5fold_3center/external_${ext}_fold${fold}.log" 2>&1
    echo "fold ${fold} / ${ext}: done -> ${log_dir}"
  done
done
echo "ALL_3CENTER_EXTERNAL_EVAL_DONE"
