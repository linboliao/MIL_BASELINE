#!/bin/bash
# External test evaluation (301 + YNZL) using the OLD (pre-SL-merge) checkpoints.
set -e
...
RESULT_ROOT=result/ProstateDiagnosis/DataAnalysis/AB_MIL_uni2_5fold_no_extsl/external_test
...
for fold in 1 2 3 4 5; do
  ...
  for ext in 301 ynzl; do
    ...
    CUDA_VISIBLE_DEVICES=${gpu} ${PY} -u test_mil.py \
      ...
      --device cuda:0 \
      > "logs/Prostate_Diagnosis/AB_MIL_uni2_5fold_sl_dev_only/old_ckpt_external_${ext}_fold${fold}.log" 2>&1
    echo "fold ${fold} / ${ext}: done -> ${log_dir}"
  done
done
echo "ALL_OLD_EXTERNAL_EVAL_DONE"
