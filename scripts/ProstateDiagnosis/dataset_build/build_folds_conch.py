...
MODEL = 'conch'
NEW_FOLDS = f'{DS}/DataAnalysis/AB_MIL_conch_5fold_3center'
SEED = 42
N_SPLITS = 5
...
for k, (tr_idx, va_idx) in enumerate(folds, start=1):
    ...
    out.to_csv(f'{d}/prostate_dev_conch_{k}fold.csv', index=False)
...
for k in range(1, N_SPLITS + 1):
    df = pd.read_csv(f'{NEW_FOLDS}/fold_{k}/prostate_dev_conch_{k}fold.csv')
