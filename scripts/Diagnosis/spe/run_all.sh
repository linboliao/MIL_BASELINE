...
readonly MIL_CONFIG_ROOT="configs/Diagnosis/MIL"
readonly INTERNAL_DATASET_ROOT="datasets/Diagnosis/MIL"
...
test_models() {
  ...
  for model in "${MODEL_NAMES[@]}"; do
    CUDA_VISIBLE_DEVICES=0 "$PYTHON_BIN" -u \
      scripts/Diagnosis/spe/test_best_checkpoints.py \
      "$@" \
...
summarize_internal() {
  ...
  "$PYTHON_BIN" -u scripts/Diagnosis/spe/summarize_performance.py \
...
run_internal_refit() {
  ...
  "$PYTHON_BIN" -u scripts/Diagnosis/spe/run.py \
    --variant "$variant" \
...
"$PYTHON_BIN" -u scripts/Diagnosis/spe/check_cohort_overlap.py \
...
"$PYTHON_BIN" -u scripts/Diagnosis/spe/run.py \
  ...
  --preflight
CUDA_VISIBLE_DEVICES=1,3,4,7 "$PYTHON_BIN" -u \
  scripts/Diagnosis/spe/run.py \
  --variant hierarchical_bacc \
