...
from utils.loop_utils import mixstyle_train_loop,val_loop
from tqdm import tqdm


def process_MIXSTYLE_AB_MIL(args):
    ...
    mixstyle_cfg = args.Model.mixstyle
    num_domains = mixstyle_cfg.num_domains
    mix_prob = mixstyle_cfg.mix_prob
    mix_alpha = mixstyle_cfg.mix_alpha
    momentum = mixstyle_cfg.momentum

    mil_model = MIXSTYLE_AB_MIL(L = L,D = D,num_classes=num_classes,dropout=dropout,act=act,in_dim=in_dim,
                                 num_domains=num_domains,mix_prob=mix_prob,mix_alpha=mix_alpha,momentum=momentum)
    mil_model.to(device)
    ...
    for epoch in tqdm(range(num_epochs),colour='GREEN'):
        ...
        train_loss,cost_time = mixstyle_train_loop(device,mil_model,train_dataloader,criterion,optimizer,now_scheduler)
