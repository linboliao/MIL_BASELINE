...
def process_AB_MIL(args):
    ...
    # NOTE: args.Dataset is an addict.Dict; attribute access on a missing key
    # auto-vivifies an empty Dict instead of raising, so use .get() (a real
    # containment check) to keep the default False for every existing config
    # that doesn't set this field.
    preload = bool(args.Dataset.get('preload', False))
    preload_mem_gb_cfg = args.Dataset.get('preload_mem_gb', None)
    mem_map = {k: int(v) for k, v in preload_mem_gb_cfg.items()} if preload_mem_gb_cfg else None
    train_dataset = WSI_Dataset(args.Dataset.dataset_csv_path,'train',preload=preload,mem_map=mem_map)
    val_dataset = WSI_Dataset(args.Dataset.dataset_csv_path,'val',preload=preload,mem_map=mem_map)
    test_dataset = WSI_Dataset(args.Dataset.dataset_csv_path,'test',preload=preload,mem_map=mem_map)
